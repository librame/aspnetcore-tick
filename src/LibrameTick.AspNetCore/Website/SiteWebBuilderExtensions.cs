...
/// 项目 Web 构建器静态扩展。
/// </summary>
public static class SiteWebBuilderExtensions
{
    ...
    public static IWebBuilder AddProjectCore<TConfiguration, TNavigation>(this IWebBuilder builder)
        where TConfiguration : SiteConfigurationBase
        where TNavigation : class, IAppllicationNavigation
    {
        builder.NotNull(nameof(builder));

        builder.AddService<IAppllicationNavigation, TNavigation>();
        ...
        where TNavigation : ProjectNavigationWithController
    {
        ...
        if (!builder.Services.TryGetAll<IAppllicationNavigation, RootProjectNavigationWithController>(out _))
            builder.Services.AddSingleton<IAppllicationNavigation, RootProjectNavigationWithController>();
    ...
    public static IWebBuilder AddProjectPage<TConfiguration, TNavigation>(this IWebBuilder builder, IMvcBuilder mvcBuilder, Assembly razorAssembly)
        where TConfiguration : SiteConfigurationBaseWithRazorPages
        where TNavigation : ProjectNavigationWithPage
    {
        builder.AddProjectCore<TConfiguration, TNavigation>();

        if (!builder.Services.TryGetAll<IAppllicationNavigation, RootProjectNavigationWithPage>(out _))
            builder.Services.AddSingleton<IAppllicationNavigation, RootProjectNavigationWithPage>();
